...
import { ref, } from 'vue'
import QuestionsListing from './QuestionsListing.vue'
...
const baseURL = ref<string>('https://opentdb.com/api.php?')
const params = ref<Params>({amount: '5', category:'', difficulty: '', type: ''})

const urlVal = ref<string>('')
const showQuestions = ref<boolean>(false)
const datas = ref<Array<any>>([])
const questionLength = ref<number>(0)
const index = ref<number>(0)
const currentQuestion = ref<Object>({})
const timer = ref<Timer>({hour: 0, minute: 10, second: 0})
...
function apiGet() {
    return new Promise((resolve, reject) => {
        let processingId = setTimeout(() => {
            console.log('answers have been found')
        }, 3000)

        console.log('Main timer', timer.value)
        // console.log(params.value)
        let url = [];
        for (let i in params.value) {
            // console.log(params.value[i])
...
function goHome() {
    ...
    console.log('home timer', timer.value)
    timer.value = { hour: 0, minute: 10, second: 0 }
    console.log('home timer change', timer.value)
}
